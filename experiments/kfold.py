...
def create_kfold(name,k,size):
    ...
    if UseGroup ==0:
        ...
        for train_index, test_index in sss.split(X, Y):
            ...
            with open('D:\\Wijayanti\\Datasets\\Data\\train_test_'+name+'_'+str(i)+'.pkl','wb') as f:
                pickle.dump([training_fix,testing_fix],f)

            i=i+1
    else: #example for cassava dataset where it has group
        
        sss = GroupShuffleSplit(n_splits=k,train_size=size, random_state=345)
        if flag==1:
            X=data.reshape(data.shape[0]*data.shape[1],data.shape[2])
            Y=gt.reshape(gt.shape[0]*gt.shape[1])
        else:
...
#create_kfold('CassavaNew',10,0.7)
#train_index,test_index=open_kfold('CassavaNew',0)
