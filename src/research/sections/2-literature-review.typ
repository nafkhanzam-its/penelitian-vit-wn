...
Penelitian-penelitian tersebut menemukan adanya korelasi yang kuat antara ciri-ciri physiologi pada tanaman dan sinyal spectral pada rentang 530 nm-550 nm untuk mendeteksi stress pada tanaman yang disebabkan karena kekurangan air pada tanaman anggur (Rapaport et al., 2015), pada rentang 600 nm-730 nm untuk mendeteksi stress karena kelebihan kandungan garam pada tanaman kedelai (Iliev et al., 2009), dan rentang 531 nm-571 nm untuk mendeteksi adanya mosaic virus pada tanaman ubi (Raji et al., 2015).
...
Moghimi dkk menggunakan ensamble feature selection untuk memilih 15 band yang paling penting dari 215 band yang tersedia yang selanjutnya band-band terpilih tadi diproses menggunakan quadratic discriminant analysis (QDA) untuk mendeteksi stress karena kelebihan garam pada tanaman (Moghimi et al., 2018).
...
Penelitian yang dilakukan oleh Godliver dkk (Owomugisha et al., 2021) menggunakan one-dimensional convolutional neural network (1D CNN) untuk mendeteksi penyakit yang disebabkan oleh virus pada tanaman ubi jalar.
Karena 1D-CNN lemah dalam mengekstraksi fitur global, Jin dkk mengusulkan untuk mengkonversi data sinyal hiperspektral ke dalam bentuk 2 dimensi dan kemudian menggunakan two-dimentioanal convolutional neural network (2D-CNN) untuk deteksi penyakit head blight pada tanaman gandum (Jin et al., 2018).
...
Oleh karena itu, Khotimah dkk mengusulkan suatu metode deep learning bernama SC-CAN yang menggunakan konvolusi 1 dimensi yang bisa mengekstraksi fitur local dan fitur global dengan cara menambahkan operasi dilasi pada proses konvolusi dan menambahkan modul channel attention untuk membuat model tersebut fokus pada informasi-informasi yang penting (Khotimah et al., 2022).
...
Sementara itu, Vaswani dkk mengusulkan suatu arsitektur deep learning yang tidak mengandung layer konvolusi sama sekali yang di sebut dengan Transformer untuk mengekstraksi fitur global dari suatu data (Vaswani et al., 2017).
...
Keberhasilan Transformer dalam menyelesaikan problem di bidang NLP, menginspirasi banyak peneliti untuk mengadopsinya untuk menyelesaikan problem-problem pada domain yang lain, seperti pada visi (Khan et al., 2022).
...
Sehingga pada penelitian kami sebelumnya, kami mengusulkan penggunaan modul spectral-to-token (S2T) dan multiscale conformer encoder (MCE) yang membuat Transformer mampu mengekstraksi fitur-fitur pada data hiperspektral walaupun jumlah data latih yang tersedia terbatas (Khotimah et al., 2023).
