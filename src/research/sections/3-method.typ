...
*Tahapan Penelitian:*
+ *Review Literatur* \
  Tahapan pertama yang sedang peneliti lakukan adalah melakukan studi literatur. Ada 2 hal utama yang peneliti pelajari di dalam studi literatur yaitu: 1) metode yang dapat merubah sinyal hiperspektral menjadi image 2) bagaimana bisa memanfaatkan ViT Transformer untuk data yang jumlah data latihnya terbatas.
+ *Pengumpulan dataset* \
...
+ *Implementasi Modul yang tepat untuk mengubah data hiperspektral ke dalam bentuk gambar.* \
  ...
  Output: gambar 2D
+ *Melakukan percobaan terhadap ViT-Transformer* \
  ViT Transformer adalah metode Transformer yang diadopsi untuk input berupa data gambar. Metode ini sangat unggul dalam menyelesaikan problem-problem yang berkaitan dengan visi computer seperti pengenalan wajah, pengenalan objek, dll. Berbeda dengan problem tersebut, kami akan mencoba mengadopsi ViT Transformer untuk mengenali gambar hasil dari modul sebelumnya.
  Input: gambar 2D
  Output: kelas dari gambar
+ *Menggabungkan modul yang diusulkan dengan ViT Transformer* \
  Karena modul STFT dan ViT Transformer pada awalnya adalah modul yang berdiri sendiri, di dalam penelitian ini kami akan mencoba menggabungkan kedua modul tersebut menjadi satu kesatuan.
  Input: sinyal hyperspectral
